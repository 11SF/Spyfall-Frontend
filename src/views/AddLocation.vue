<template>
  <div class="">
    <header
      class="bg-[#1F4690] text-white w-full h-20 shadow-sm flex justify-between items-center px-10"
    >
      <button class="" @click="backtoHomePage">ย้อนกลับ</button>
      <button class="bg-[#FFA500] h-12 w-20 rounded-xl shadow">บันทึก</button>
    </header>
    <div class="container mx-auto pt-10">
      ...
      <section class="mb-5" v-if="!getCreateLocationStatus">
        <LocationList />
      </section>

      <section v-if="getCreateLocationStatus">
...
export default {
  ...
  setup() {
    ...
    const { fetchLocation, rollbackLocation, clearNewLocation } =
      useLocationStore();
